...
# points = gpd.read_file('C:\Geotar\CHAD\geodata\workspace\Village_Baseline_FCS_rCSI_06_02.shp')

# # create a new column with a shapely Point object for each point
# # calculate buffer around each point
# buffer_distance = 0.05 # units are based on the CRS of the input shapefile
# buffered = points.copy()
# buffered['geometry'] = points['geometry'].buffer(buffer_distance)

# # create a new GeoDataFrame with the buffered geometries and original attributes
# buffered_points = gpd.GeoDataFrame(buffered, geometry='geometry', crs=points.crs)
# buffered_points.head()


# Alternative for polygon input files, the input file must be cropped using the mask of the pilot

# In[8]:


# Read in the polygon shapefile and the mask shapefile
#polygons_path = f'C:/Geotar/{pilot}/geodata/workspace/Village_bound_manual.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/workspace/veredas_sel.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/Processed/boundaries/Najaf_adm3.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/Processed/boundaries/Dahuk_adm3.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/Processed/boundaries/Iraq_mask.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/Raw/boundaries/lbn_bnd_cadastral_unhcr_wfpco.shp'
#polygons_path = f'C:/Geotar/{pilot}/geodata/workspace/No_data_pols.shp'
buffered_points = gpd.read_file(polygons_path)

# Select the polygons in poly_shp that are completely contained in mask_shp
#buffered_points = buffered_points [buffered_points.within(mask.unary_union)]
buffered_points.head()
# If you want to write the resulting GeoDataFrame to a shapefile:
#buffered_points.to_file(f"C:/Geotar/{pilot}/geodata/workspace/veredas_sel.shp")


# Shows on the map the location of the polygons that will be aggregated
...
# add Google basemap layer to the map
folium.TileLayer(
...
# Add the GeoDataFrame with the buffer to the map
folium.GeoJson(buffered_points).add_to(m)
folium.GeoJson(mask, style_function=lambda feature: {'color': 'red', 'weight': 2,}).add_to(m)
# Display the map
m
...
#buffered = gpd.read_file('buffered.shp')
...
for name, data in rasters.items():
    ...
    # buffered_points.plot(ax=plt.gca(), facecolor='none', edgecolor='red')
...
# #buffered.to_file('buffered_with_mean.shp')
...
# Extract the mean LST values for each polygon
    ...
    for i, poly_ntl in enumerate (buffered_points['geometry']):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                masked_data_ntl = data_ntl[mask_ntl]
                max_value_ntl = np.max(masked_data_ntl)
        # print(mean_value_ntl)
        # check if the masked_data_ntl array is empty
        # if len(masked_data_ntl) > 0:
        #     # calculate the mean value of the masked raster data
        #     mean_value_ntl = np.max(masked_data_ntl)
        #     #print("masked_data_ntl > 0")
        # else:
        #     # Handle the case where the array is empty (you can choose an appropriate value)
        #     mean_value_ntl = 0.0  # or any other default value
        #     #print("Error")
        # add the mean value to the list for this raster
